...
const help: typeof enUS = {
  ...
  finding: {
    ...
    sort: {
      label: "自由に並べ替え",
      caption:
        "新着順、評価が高い順、スクリプト再生数順などで並び替えられます。隣の矢印ボタンで昇順と降順が入れ替わります"
    },
    tags: {
      label: "タグクラウド",
      caption:
        "ミュート中のものを除く、カタログのすべてのタグを検索・並び替えできます。クリックすると一覧ページがそのタグで絞り込まれます"
    },
    performers: {
      label: "出演者",
  ...
  library: {
    ...
    bulkDownload: {
      label: "スクリプトを一括取得",
      caption:
        "プレイリストで1回クリックすると、含まれている無料スクリプトをすべてダウンロードします"
    },
    quickMenu: {
      label: "クイックメニュー",
      caption:
  ...
  scripts: {
    ...
    free: {
      label: "無料スクリプト",
      caption:
        "「無料」と表示された動画のスクリプトは、Handyの接続キーがあればダウンロードできます"
    },
    rate: {
      label: "スクリプトを評価",
      caption: "無料スクリプトは動画ページでそのまま星をつけて評価できます"
    },
    comments: {
      label: "スクリプトへのコメント",
      caption: "スクリプトへの匿名コメントを読んだり投稿したりできます"
    },
    requests: {
      label: "動画をリクエスト",
      caption:
        "好きな動画のスクリプト作成を依頼し、次に作るものを投票で決められます"
    }
  },
  ...
  personalize: {
    ...
    filters: {
      label: "性的指向・スクリプト・動画のフィルター",
      caption:
        "スクリプトと動画それぞれの無料・有料、そして出演者の性的指向を、設定からでも一覧のフィルターからでも指定できます"
    },
    mutedTags: {
      label: "ミュート中のタグ",
      caption:
