		...
		<template v-if="false">
			...
			<p>
				...
				<br><br><b>NB! We removed the ability to read a URL for now. You must go to the index site and manually
					request
					the
					URL. We might change it back to be able to read the URL, but it felt wrong to read the URLs even if
					we
					did
					not use it.</b>
			<ul>
				<li>
					...
					scramble any
					sensitive user data (IP, in this case). Since the backend code is not open source, you have to take
					our
					word for it.
				</li>
				<li>The browser plugin is open source to fully transparent what's happening behind the scenes.</li>
				<a href="https://github.com/poengAlex/handy-browser-extension" target="_blank">Github repo of the
					browser
					extension</a>

				<li>We do not care what you are watching since this data would not give us much value and would increase
					our
					risk if we have a security breach.</li>
				<li>We only send a query on the supported sites, not every page you visit.</li>
				<li>We also use the url if you request us to script the website you are visiting. This request is stored
					in
